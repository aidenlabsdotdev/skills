...
- **An MCP / SDK that abstracts the browser away** — check its session-listing tool for a `cdp_url` (or `cdp_endpoint`) field per session and pass that. If it doesn't expose one, fall back to one of the next options.
